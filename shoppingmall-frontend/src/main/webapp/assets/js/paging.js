function setPage(listCount, currentPage, kwd) {
        ...
        var pager = $('#pager');
        if(currentPage <= 5){
        	pager.prepend('<li><a class="np-btn" href="#"><i class="fa fa-angle-left  pagination-left"></i></a></li>');
		}else{
			pager.append('<li>'+
							'<a class="np-btn" href="${pageContext.servletContext.contextPath }/admin/product/list/'+(displayPage-5)+'?keyword='+kwd+'">'+
        ...
        for (var i = displayPage-4; i <= displayPage; i++) {
			...
			pager.append('<li class="page-item">'+
							'<a class="np-btn" href="${pageContext.servletContext.contextPath }/admin/product/list/'+i+'?keyword='+kwd+'">'+
							'</a>'+
						'</li>')
		...
		if(currentPage < endPage && endPage < pageCount){
			pager.append('<li>'+
							'<a class="np-btn" href="${pageContext.servletContext.contextPath }/admin/product/list/'+nextPage+'?keyword='+kwd+'">'+
